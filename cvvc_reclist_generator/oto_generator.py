    ...
    def gen_oto(self, reclist: Reclist, alias_union: AliasUnion, bpm: float) -> None:
        ...
        for row in reclist:
            ...
            if len(row) == 3 and row[0] == row[1] == row[2]:
                ...
                if (
                    cv_head := Alias(row[0].get_cv(IS_FULL_CV), AliasType.CV_HEAD)
                ...
                if (
                    cv := Alias(row[1].get_cv(IS_FULL_CV), AliasType.CV)
                ) in alias_union.cv:
                ...
                if (
                    vc := Alias((row[0].get_v(USE_END_V), row[1].c), AliasType.VC)
                ) in alias_union.vc:
                ...
                if (
                    vcv := Alias(
                        (row[0].get_v(USE_END_V), row[1].get_cv(IS_FULL_CV)),
                        AliasType.VCV,
                    )
                ) in alias_union.vcv:
                ...
                if (vr := Alias(row[2].get_v(USE_END_V), AliasType.V)) in alias_union.v:
                ...
                if (
                    USE_END_V
                    and (vc := Alias((row[0].mid_v, row[0].end_v), AliasType.VC))
                    in alias_union.vc
                ):
                    vc_oto = self.get_oto(wav, vc, 0, bpm)
                    self.oto_union.VC.append(vc_oto)
                    alias_union.vc.discard(vc)
            else:
                for idx, cvv in enumerate(row):
                    if cvv == self.EMPTY_CVV:
                        continue
                    if idx == 0:
                        if (
                            ...
                            self.oto_union.C.append(c_head_oto)
                            alias_union.c_head.discard(c_head)
                        cv_head = Alias(cvv.get_cv(IS_FULL_CV), AliasType.CV_HEAD)
                        ...
                        if (
                            len(row) == 1
                            and (vr := Alias(cvv.get_v(USE_END_V), AliasType.V))
                            in alias_union.v
                        ):
                        ...
                        if row[idx + 1] == self.EMPTY_CVV:
                            if (
                                vr := Alias(cvv.get_v(USE_END_V), AliasType.V)
                            ) in alias_union.v:
                                oto = self.get_oto(wav, vr, idx, bpm)
                                self.oto_union.V.append(oto)
                                alias_union.v.discard(vr)
                            ...
                            if (
                                vc := Alias(
                                    (cvv.get_v(USE_END_V), row[idx + 1].c),
                                    AliasType.VC,
                                )
                            ) in alias_union.vc:
                        ...
                        if row[idx - 1] == self.EMPTY_CVV:
                            if (
                                cv_head := Alias(
                                    cvv.get_cv(IS_FULL_CV), AliasType.CV_HEAD
                                )
                            ) in alias_union.cv_head:
                                oto = self.get_oto(wav, cv_head, idx, bpm)
                                self.oto_union.CV.append(oto)
                            ...
                            if (
                                vc := Alias(
                                    (row[idx - 1].get_v(USE_END_V), cvv.c), AliasType.VC
                                )
                            ) in alias_union.vc:
                                oto = self.get_oto(wav, vc, idx, bpm)
                                self.oto_union.VC.append(oto)
                        ...
                        if row[idx - 1] == self.EMPTY_CVV:
                            ...
                            if (
                                ...
                                oto = self.get_oto(wav, vc, idx - 1, bpm)
                                self.oto_union.VC.append(oto)
                                alias_union.vc.discard(vc)
                            if (
                                cv := Alias(cvv.get_cv(IS_FULL_CV), AliasType.CV)
                    ...
                    if (
                        USE_END_V
                        and (
                            vc := Alias((cvv.mid_v, cvv.get_v(USE_END_V)), AliasType.VC)
                        )
                        in alias_union.vc
                    ):
                        vc_oto = self.get_oto(wav, vc, idx, bpm)
                        self.oto_union.VC.append(vc_oto)
                        alias_union.vc.discard(vc)
